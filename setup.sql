...
insert into students values ('110','Jon',4,'IS',4.0,4.0);
insert into students values ('111','Kat',4,'IS',4.0,4.0);
insert into students values ('112','Liam',4,'IS',4.0,4.0);
insert into students values ('113','Mathew',4,'IS',4.0,4.0);
...
insert into enrollments values (103,10170,null);
...
declare
	p_error_message varchar2(1000);
begin
	Enroll.AddMe(109, 10140, p_error_message);
end;
/
...
--check successful waitList
declare
	p_error_message varchar2(1000);
begin
	Enroll.AddMe(110, 10150, p_error_message);
end;
/
...
declare
	p_error_message varchar2(1000);
begin
	Enroll.AddMe(113, 10150, p_error_message);
end;
/

--check repeat waitlist successfully added
declare
	p_error_message varchar2(1000);
begin
	Enroll.AddMe(107, 10110, p_error_message);
end;
/
...
if v_count = 0 then
	 p_error_text := 'Error!, student number '||p_snum||' is not valid. ';
...
if v_count = 0 then
	p_error_text := 'Error!, the class '||p_callnum||' is not invalid. ';
...
commit;
if v_count > 0 then
	p_error_text := ' Error!, repeat enrollment. ';
end if;
end;
...
if v_count != 0 then
	p_error_text := ' Error!, double enrollment, you have already in this course in section number. ';
end if;
end;
...
if v_CrHrAdded+v_CrHrCheck>=15 then
	p_error_text := ' Error!, 15-Hour-Rule, the credit hours cannot exceed 15 units per semester. ';
end if;
end;
...
if v_studentStanding <= v_courseStanding then
	p_error_text := ' Error!, Student does not meet the standing requirements. ';
end if;
end;
...
and v_studentGPA <= 2 then
	...
	p_error_text :=  ' Error!, disqualified status, Student does not meet the GPA requirement of 2.0 or more. ';
else
	p_error_text := null;
end if;
end;
...
if v_count > v_capacity then
	p_error_text := ' Error!, capacity for '||p_callnum||' is full. ';
end if;
commit;
end;
...
if SQL%FOUND then
	p_error_text:= 'Congrats!, you are dropped from the waitlist. ';
else
	p_error_text:= 'Error!, Student was not removed from waitlist';
end if;
end;
...
if v_count = 1 then
	p_error_text:=' Error!, Student is already on the waitlist for this class. ';
else
	p_error_text:= null;
...
if v_count = 0 then
	p_error_text:= ' Error!, student is not enrolled in this class. Error!, Student cannnot drop it. ';
else
	p_error_text:= null;
end if;
...
if v_count = 1 then
	p_error_text:= ' Error!, Student cannot drop, you have already have a letter grade for this class. ';
else
	p_error_text:= null;
end if;
end;
...
if p_errMsg is null then
	...
	if p_errMsg is null then
		...
		for eachRecord in (
			select snum, callnum from waitlist
			where callnum=p_callnum
